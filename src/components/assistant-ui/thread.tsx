...
import { useQueuedMessageIds } from "@/components/chat-provider";
import {
  Collapsible,
  CollapsibleContent,
  CollapsibleTrigger,
...
const UserMessage: FC = () => {
  const queuedIds = useQueuedMessageIds();
  const messageId = useMessage((m) => m.id);
  const isQueued = queuedIds.has(messageId);

  return (
    <MessagePrimitive.Root
      className={cn(
        "aui-user-message-root group/message fade-in slide-in-from-bottom-1 mx-auto grid w-full max-w-(--thread-max-width) animate-in auto-rows-auto grid-cols-[minmax(72px,1fr)_auto] content-start gap-y-2 px-2 py-3 duration-150 [&:where(>*)]:col-start-2",
        isQueued && "opacity-60"
      )}
      data-role="user"
    >
      ...
      <div className="aui-user-message-content-wrapper relative col-start-2 min-w-0">
        <div className={cn(
          "aui-user-message-content wrap-break-word rounded-2xl bg-muted px-4 py-2.5 text-foreground",
          isQueued && "border border-dashed border-amber-400/50"
        )}>
          <MessagePrimitive.Parts />
        </div>
      ...
      {/* Queued badge or timestamp */}
      <div className="col-start-2 -mt-1 flex justify-end pr-0.5">
        {isQueued ? (
          <span className="inline-flex items-center gap-1 text-[10px] font-medium text-amber-500 dark:text-amber-400 select-none">
